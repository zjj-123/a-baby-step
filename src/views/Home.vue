...
export default {
  ...
  mounted() {
    ...
    document
      ...
      .addEventListener('mousemove', this.listenerMousemove);
    this.$once('hook:beforeDestroy', () => {
      document
        .querySelector('.calendar')
        .removeEventListener('mousemove', this.listenerMousemove);
    });
  },
  methods: {
    listenerMousemove(e) {
      ...
      this.borderLayer.style.webkitMaskPosition = `${left - 80}px ${top -
        80}px`;
    },
    getBorderLayerInfo() {
      // 获取网格信息以及在视窗中的位置信息
